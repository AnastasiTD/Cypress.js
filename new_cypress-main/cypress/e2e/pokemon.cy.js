import * as log from "../helpers/log.json"

describe('Проверка покупки нового аватара', function () {                 // название набора тестов
    it('e2e тест на покупку нового аватара для тренера', function () {   // название теста
         cy.visit('https://pokemonbattle.ru/');                          // переходим на сайт https://pokemonbattle.ru/
         cy.get('input[id="k_email"]').type(log.login);                   // вводим логин
         cy.get('input[id="k_password"]').type(log.password);               // вводим пароль
         ...
         cy.get('.card_date').type('12/34');                           // вводим срок действия карты
         cy.get('.card_name').type('тоом анастасия');                           // вводим имя владельца действия карты
